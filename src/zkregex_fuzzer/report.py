...
class Stats:
    ...
    def __init__(self, results: list[tuple[str, list[str], HarnessResult]]):
        self.regexes, self.inputs, self.results = zip(*results)

    def get_stats(self):
        return {
            "regexes": len(self.regexes),
            "total_inputs": sum([len(inputs) for inputs in self.inputs]),
            "avg_inputs": sum([len(inputs) for inputs in self.inputs])
            / len(self.regexes),
            "min_inputs": min([len(inputs) for inputs in self.inputs]),
            "max_inputs": max([len(inputs) for inputs in self.inputs]),
            "total_errors": sum(
                [result.status != HarnessStatus.SUCCESS for result in self.results]
            ),
            "total_valid": sum(
                [result.status == HarnessStatus.SUCCESS for result in self.results]
            ),
            "total_oracle_violations": sum(
                [result.status == HarnessStatus.FAILED for result in self.results]
            ),
            ...
            "total_run_errors": sum(
                [result.status == HarnessStatus.RUN_ERROR for result in self.results]
            ),
            "total_invalid_seed": sum(
                [result.status == HarnessStatus.INVALID_SEED for result in self.results]
            ),
        }
